    ...
    public class Population
    {
        ...
        public IFitnessFunction FitnessFunction
        {
            ...
            set
            {
                ...
                if (UseParallel)
                {
                    population.AsParallel().ForAll(x => x.Evaluate(fitnessFunction));
                }
                else
                {
                    population.ForEach(x=>x.Evaluate(fitnessFunction));
                }
                FindBestChromosome();
            }
        }
                           ...
                           ISelectionMethod selectionMethod )
        {
            ...
            var clone = ancestor.Clone();
            List<IChromosome> newChromosomes = new List<IChromosome> {clone};
            ...
            if (UseParallel)
            {
                newChromosomes.AsParallel().ForAll(x => x.Evaluate(fitnessFunction));
            }
            else
            {
                newChromosomes.ForEach(x => x.Evaluate(fitnessFunction));
            }
        ...
        public void Resize( int newPopulationSize, ISelectionMethod membersSelector )
        {
            ...
            if ( newPopulationSize > size )
            {
                ...
                List<IChromosome> newChroosomes = new List<IChromosome>();
                ...
                if (UseParallel)
                { 
                newChroosomes.AsParallel().ForAll(x=>x.Evaluate(fitnessFunction));
                }
                else
                {
                    newChroosomes.ForEach(x => x.Evaluate(fitnessFunction));
                }
                population.AddRange(newChroosomes);
            }
            else
            {
                // do selection
